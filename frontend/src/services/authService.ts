...
export interface ClassPeopleData {
  teacher: User | null;
  students: User[];
  class_name: string;
...
export const getClassPeople = async (classId: string | number): Promise<ClassPeopleData> => {
  const response = await apiClient.get(`/api/classes/${classId}/people`);
  return response.data;
};
