                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                  ...
                  <div className="flex items-center gap-2 text-xs lg:text-sm text-gray-600">
                    ...
                    {currentUser?.role === 'admin' && (
                      <button
                        onClick={refreshClassList}
                        className="inline-flex items-center gap-1 px-3 py-1 bg-white hover:bg-gray-50 text-gray-700 rounded-lg transition-all duration-300 text-xs border border-gray-300 font-medium cursor-pointer"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-6">
                    {filteredClasses.map((classItem, index) => {
                      ...
                      return (
                        <div
                          key={classItem.id}
                          onClick={() => navigate(`/class/${classItem.id}/details`)}
                          className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden group hover:-translate-y-1 cursor-pointer"
                        >
                          {/* Class Header with Color */}
                          <div className={`h-24 ${color.bg} relative p-4 transition-colors duration-300`}>
                            <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent"></div>

                            {/* Actions overlay */}
                            <div className="absolute top-0 right-0 p-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                              {/* Make specific actions stop propagation */}
                              {currentUser?.role === 'admin' && (
                                <div className="flex gap-2">
                                  <button
                                    ...
                                    className="w-8 h-8 bg-white/20 backdrop-blur-sm hover:bg-white/30 rounded-lg flex items-center justify-center transition-colors cursor-pointer"
                                  ...
                                  <button
                                    ...
                                    className="w-8 h-8 bg-white/20 backdrop-blur-sm hover:bg-red-500/30 rounded-lg flex items-center justify-center transition-colors cursor-pointer"
                            ...
                            {/* Card Content - ensure clicks bubble up */}
                            <div className="relative z-10 w-full h-full flex flex-col justify-end">
                              <h3 className="text-xl font-bold text-white mb-1 truncate shadow-sm">{classItem.name}</h3>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="flex items-center justify-between">
                                <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-full border border-emerald-200">
                                  <span className={`w-2 h-2 rounded-full animate-pulse ${classItem.status === 'Inactive' ? 'bg-gray-400' : 'bg-emerald-500'}`}></span>
                                  <span className="text-xs font-medium">{classItem.status || 'Active'}</span>
                                </div>
                              ...
                              <div className="pt-3 border-t border-gray-100">
                                <div className="flex items-center justify-between gap-2">
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      navigate(`/class/${classItem.id}/details`);
                                    }}
                                    className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg transition-all duration-300 text-xs border border-blue-200 font-medium cursor-pointer"
                                    title={`View ${classItem.name}`}
                                  >
                                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  ...
                                  {currentUser?.role === 'admin' && (
                                    <button
                                      ...
                                      className="inline-flex items-center gap-2 px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg transition-all duration-300 text-xs border border-red-200 font-medium cursor-pointer"
                                      title={`Delete ${classItem.name}`}
                                    >
                                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                      ...
                      currentUser?.role === 'admin' && (
                        ...
                        >
                          <div className="h-full min-h-[240px] flex flex-col items-center justify-center p-6">
                            ...
                            <h3 className="text-lg font-semibold text-gray-900 mb-2">Create Class</h3>
