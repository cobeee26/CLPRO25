...
const StudentDashboard: React.FC = () => {
  ...
  const handleJoinSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post('/api/enrollment/join', { join_code: joinCode });

      showSuccessAlert('Success!', 'You have successfully joined the class.', 'refresh', true, 2000);
      ...
      const msg = error.response?.data?.detail || "Failed to join class. Please try again.";
      setJoinError(msg);
      showErrorAlert('Join Failed', msg, true, 4000);
    } finally {
    ...
    <div className="h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 flex overflow-hidden">
      ...
      {isJoinModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-auto border border-gray-300">
            ...
            <form onSubmit={handleJoinSubmit} className="p-6">
              <div className="mb-6">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-4 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-mono text-lg tracking-widest uppercase placeholder-gray-400"
                    maxLength={6}
                ...
                <p className="mt-2 text-xs text-gray-500">
                  Ask your teacher for the join code to enroll in their class.
                </p>
                {joinError && (
                  <div className="mt-3 p-3 bg-red-50 text-red-600 text-sm rounded-lg flex items-center animate-shake">
              ...
              <div className="flex justify-end space-x-3">
                <button
                  type="button"
                  onClick={() => setIsJoinModalOpen(false)}
                  className="px-5 py-2.5 text-gray-600 font-medium hover:bg-gray-50 rounded-xl transition-colors disabled:opacity-50 cursor-pointer"
                  disabled={joinLoading}
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-medium rounded-xl hover:shadow-lg hover:scale-[1.02] transition-all disabled:opacity-50 disabled:hover:scale-100 flex items-center cursor-pointer"
