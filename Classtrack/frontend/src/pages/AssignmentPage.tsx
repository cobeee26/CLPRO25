...
const DashboardPage: React.FC = () => {
  ...
  // Recent activities state - UPDATED: Only 'user' and 'class' types
  ...
  // Fetch recent activities from API - UPDATED: Only gets latest user and latest class
  const fetchRecentActivities = async (usersData: any[] = [], classesData: any[] = []) => {
    try {
      setActivitiesLoading(true);
      
      // If users data wasn't provided, fetch it
      if (usersData.length === 0) {
        usersData = await getAllUsers();
      }
      
      // If classes data wasn't provided, fetch it
      if (classesData.length === 0) {
        classesData = await getAllClasses();
      }
      
      // Get latest user (newest by id)
      let latestUser = null;
      if (usersData.length > 0) {
        // Sort by id in descending order to get the newest
        const sortedUsers = usersData.sort((a, b) => b.id - a.id);
        // Take the latest user
        latestUser = sortedUsers[0];
      }
      
      // Get latest class (newest by id)
      let latestClass = null;
      if (classesData.length > 0) {
        // Sort by id in descending order to get the newest
        const sortedClasses = classesData.sort((a, b) => b.id - a.id);
        // Take the latest class
        const classToProcess = sortedClasses[0];
        
        if (classToProcess) {
          let teacherUsername = 'Unassigned';
          
          // Try to get teacher username from class data
          if (classToProcess.teacher_id) {
            // Find teacher from users data
            const teacher = usersData.find(user => user.id === classToProcess.teacher_id);
            if (teacher) {
      ...
      // Build recent activities array - ONLY latest user and latest class
      ...
      if (latestUser) {
        activities.push({
          ...
          description: `Username "${latestUser.username}" joined the system`,
          timestamp: twoMinutesAgo.toISOString(),
          timeAgo: formatTimeAgo(twoMinutesAgo)
        });
      }
      
      // Add class creation activity if available
      if (latestClass) {
        activities.push({
          id: 2,
          type: 'class',
          title: 'Class created',
          description: `"${latestClass.name}" created by "${latestClass.teacher_username}"`,
          timestamp: fifteenMinutesAgo.toISOString(),
          timeAgo: formatTimeAgo(fifteenMinutesAgo)
        });
      }
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {activitiesLoading ? (
                      ...
                      recentActivities.map((activity) => (
                        <div 
                          key={activity.id} 
                          className="group flex items-center space-x-3 sm:space-x-4 p-3 sm:p-4 bg-gray-50 hover:bg-gray-100 rounded-xl sm:rounded-2xl transition-all duration-300 hover:shadow-lg border border-gray-200 cursor-pointer"
                        >
                          <div className="relative">
                            <div className={`absolute inset-0 ${getActivityBgLightColor(activity.type)} rounded-xl sm:rounded-2xl`}></div>
                            <div className={`relative w-10 h-10 sm:w-12 sm:h-12 ${getActivityBgColor(activity.type)} rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg`}>
                              {getActivityIcon(activity.type)}
                            </div>
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs sm:text-sm font-bold text-gray-900 mb-1">{activity.title}</p>
                            <p className="text-xs text-gray-600 font-medium truncate">{activity.description}</p>
                          </div>
                          <div className="text-right flex-shrink-0">
                            <span className="text-xs text-gray-500 font-medium bg-gray-100 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full">
                              {activity.timeAgo}
                            </span>
                          </div>
                        </div>
