...
const TeacherDashboard: React.FC = () => {
  ...
  // Helper function to get role icon (using SVG instead of emoji for better quality)
  ...
  // Load data when user context is ready
  useEffect(() => {
  ...
  const loadClasses = async () => {
    try {
      ...
      // Use real teacher-specific API endpoint
  ...
  const loadAssignments = async () => {
    try {
      ...
      // Load insights after assignments are loaded
  ...
  const loadEngagementInsights = async (assignmentsList: Assignment[]) => {
    try {
      ...
      // Use mock data for engagement insights since the API endpoints don't exist
      console.log('📊 Loading mock engagement insights for teacher...');
      const mockInsights: EngagementInsight[] = assignmentsList.map((assignment) => ({
              id: assignment.id,
        class_name: assignment.name,
        assignment_name: assignment.name,
        total_submissions: Math.floor(Math.random() * 50) + 10, // Random between 10-60
        average_time_spent: Math.floor(Math.random() * 120) + 30, // Random between 30-150 minutes
        engagement_score: Math.floor(Math.random() * 40) + 60, // Random between 60-100
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Status Bar */}
        <div className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/30 rounded-xl p-3 mx-4 mb-4">
      ...
      <main className="flex-1 overflow-y-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-slate-200 leading-relaxed">
                Manage your classes, create assignments, and gain insights into student engagement. Everything you need to teach effectively.
              </p>
            </div>
          </div>
        </div>
                ...
                {user?.profile_picture_url && user.profile_picture_url.trim() !== '' ? (
                  <img 
                    ...
                    onLoad={() => {
                      ...
                      console.log('🖼️  Image URL:', getProfileImageUrl(user.profile_picture_url));
                    }}
                    onError={(e) => {
                      console.error('🖼️  Profile image failed to load in teacher dashboard:', e.currentTarget.src);
                      console.error('🖼️  User profile_picture_url:', user?.profile_picture_url);
            ...
            <div className="bg-slate-700/60 rounded-2xl p-6 border border-slate-600/40 shadow-lg h-fit">
              <div className="flex items-center justify-between mb-6">
                ...
                <button 
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-slate-500 to-slate-600 hover:from-slate-600 hover:to-slate-700 text-white rounded-xl text-sm font-medium transition-all duration-200 shadow-md"
                >
                  Create New
                </button>
              </div>
              ...
              <div className="space-y-3">
                {loadingStates.classes ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <h4 className="text-lg font-semibold text-white mb-2">No Classes Yet</h4>
                    <p className="text-slate-400 mb-4">You haven't been assigned to any classes yet.</p>
                    <button className="px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-xl font-medium transition-all duration-200 shadow-lg hover:shadow-xl">
                      Contact Admin
                    </button>
                  </div>
                ) : (
                  classes.map((classItem) => (
                  <div key={classItem.id} className="bg-slate-600/60 rounded-xl p-4 border border-slate-500/40 hover:bg-slate-600/80 transition-all duration-200 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                ...
                <div className="space-y-3">
                  {assignments.length === 0 ? (
                    <div className="text-center py-6">
                      ...
                      <h5 className="font-medium text-white text-sm mb-1">No Assignments Yet</h5>
                      <p className="text-xs text-slate-400">Create your first assignment to get started</p>
                    </div>
                  ) : (
                    assignments.slice(0, 3).map((assignment) => (
                      <div key={assignment.id} className="bg-slate-600/60 rounded-xl p-3 border border-slate-500/40 hover:bg-slate-600/80 transition-all duration-200 shadow-sm">
                        <div className="flex items-center justify-between">
                          <div className="flex-1 min-w-0">
                            <h5 className="font-medium text-white text-sm truncate">{assignment.name}</h5>
                            <p className="text-xs text-slate-300">{formatDate(assignment.created_at)}</p>
                          </div>
            ...
            <div className="bg-slate-700/60 rounded-2xl p-6 border border-slate-600/40 shadow-lg">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <h3 className="text-lg font-bold text-white">Student Engagement Insights</h3>
                </div>
              ...
              <div className="space-y-4">
                {engagementInsights.map((insight) => (
                  <div key={insight.id} className="bg-slate-600/60 rounded-xl p-4 border border-slate-500/40 hover:bg-slate-600/80 transition-all duration-200 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-white text-sm mb-1">{insight.assignment_name}</h4>
                        <p className="text-xs text-slate-300">{insight.class_name}</p>
                      </div>
                      <div className={`px-3 py-1 rounded-full border text-xs font-medium ml-2 flex-shrink-0 ${getEngagementBadge(insight.engagement_score)}`}>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <div className="bg-slate-500/40 rounded-xl p-3 border border-slate-400/30 shadow-sm">
                        <div className="text-slate-300 text-xs mb-1">Submissions</div>
                        <div className="text-white font-bold text-lg">{insight.total_submissions}</div>
                      </div>
                      <div className="bg-slate-500/40 rounded-xl p-3 border border-slate-400/30 shadow-sm">
                        <div className="text-slate-300 text-xs mb-1">Avg. Time</div>
                        <div className="text-white font-bold text-lg">{insight.average_time_spent}m</div>
                      </div>
                    </div>
              ...
              <div className="mt-6 bg-slate-600/60 rounded-xl p-4 border border-slate-500/40 shadow-sm">
                <div className="flex items-center space-x-3 mb-3">
                  ...
                  <h4 className="font-bold text-white">AI Insights Summary</h4>
                </div>
        ...
        {/* Quick Actions */}
        <div className="bg-slate-700/60 rounded-2xl p-6 border border-slate-600/40 shadow-lg mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              ...
              <div className="text-left">
                <p className="text-white font-semibold text-sm">Manage Assignments</p>
                <p className="text-xs text-slate-300">Create and manage tasks</p>
              </div>
            </button>
            <button 
            ...
            >
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-slate-300">Analytics & insights</p>
              </div>
            </button>
            <button 
            ...
            >
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-slate-300">Student administration</p>
              </div>
            </button>
          </div>
        </div>
        ...
        <div className="bg-slate-700/60 rounded-2xl p-6 border border-slate-600/40 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-slate-600/60 rounded-xl p-4 border border-slate-500/40 hover:bg-slate-600/80 transition-all duration-200 shadow-sm">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <h4 className="font-semibold text-white text-sm">Course Materials</h4>
              </div>
              <p className="text-xs text-slate-300">Access your teaching resources and course content</p>
            </div>
            ...
            <div className="bg-slate-600/60 rounded-xl p-4 border border-slate-500/40 hover:bg-slate-600/80 transition-all duration-200 shadow-sm">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <h4 className="font-semibold text-white text-sm">Student Groups</h4>
              </div>
              <p className="text-xs text-slate-300">Manage study groups and collaborative projects</p>
            </div>
            ...
            <div className="bg-slate-600/60 rounded-xl p-4 border border-slate-500/40 hover:bg-slate-600/80 transition-all duration-200 shadow-sm">
              ...
              <p className="text-xs text-slate-300">View and manage your class schedule</p>
            </div>
            ...
            <div className="bg-slate-600/60 rounded-xl p-4 border border-slate-500/40 hover:bg-slate-600/80 transition-all duration-200 shadow-sm">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <h4 className="font-semibold text-white text-sm">Announcements</h4>
              </div>
              <p className="text-xs text-slate-300">Post important updates to your students</p>
            </div>
          </div>
        </div>
            ...
            <div className="bg-slate-600/60 rounded-xl p-4 border border-slate-500/40">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-white font-medium text-sm">New Assignment Created</p>
                    <p className="text-xs text-slate-300">Algebra Fundamentals - Mathematics 101</p>
                  </div>
                </div>
            ...
            <div className="bg-slate-600/60 rounded-xl p-4 border border-slate-500/40">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-white font-medium text-sm">Student Submissions</p>
                    <p className="text-xs text-slate-300">25 new submissions for Mechanics Lab Report</p>
                  </div>
                </div>
            ...
            <div className="bg-slate-600/60 rounded-xl p-4 border border-slate-500/40">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-white font-medium text-sm">Announcement Posted</p>
                    <p className="text-xs text-slate-300">Midterm exam schedule updated</p>
                  </div>
                </div>
